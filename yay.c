...
int load_frame(){
  ...
  if(cnt < width*height){
    ...
    cnt = fread(cb_data, 1, height * width / 4, fpointer);
    // fprintf(stderr,"read %d cb bytes\n", cnt);
    if(cnt < width * height / 4){
      return 0;
    } else {
      cnt = fread(cr_data, 1, height * width / 4, fpointer);
...
void print_usage(){
  fprintf(stdout, "Usage: yay [-s <widht>x<heigh>] filename.yuv\n");
}
...
int main(int argc, char *argv[])
{
  ...
  if(argc == 2){
	...
	if (sscanf(optarg, "%dx%d", &width, &height) != 2) {
	  fprintf(stdout, "No geometry information found in path/filename or provided by -s paramter.\n");
	  return 1;
	}
      }
    vfilename = argv[3];
  ...
  cb_data = malloc(width * height * sizeof(Uint8) / 4);
  cr_data = malloc(width * height * sizeof(Uint8) / 4);
	...
	switch(event.key.keysym.sym)
	  ...
	  case SDLK_LEFT:
	    { 
	      if(frame>1){
		frame--;
		fseek(fpointer, (frame-1)*1.5*height*width , SEEK_SET);
		//if(draw_frame())
		load_frame();
		draw_frame();
	      }
